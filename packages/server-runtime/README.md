...
const config = core.register({
  ...
  pack: 'bc_shop',                // abbreviated pack id — [a-z0-9_]+
  packName: 'My Cool Shop',       // display label only — not part of identity
  version: '1.0.0',
  description: 'Sells items',
  dependencies: ['os_bc_economy'],      // namespaces you need (soft — warns, never blocks)
  optionalDependencies: ['os_bc_leaderboard'], // namespaces that unlock optional features
  translations: translationKeys, // optional — generated by-locale keys (see Translations)
...
console.warn(core.id);          // 'ms_bc_shop' — this addon's namespace
...
`ms_bc_shop`) to form **the** identifier the addon is known by.
...
- A creator shipping several addons uses distinct `pack` ids (`ms_bc_shop`, `ms_bc_economy`) —
...
identity. `register()` validates the manifest and throws on a bad manifest or a second call.
...
core.registry.get('os_bc_economy'); // by namespace
core.registry.has('os_bc_economy'); // is it present?
...
```ts
core.registry.missingDependencies();           // ['os_bc_economy'] until it registers
...
core.features.add('leaderboard-sync', {
  ...
  condition: ctx => ctx.registry.has('os_bc_leaderboard'),
...
core.features.add('cross-pvp', {
  condition: ctx =>
    ctx.registry.has('os_bc_pvp') &&
    ctx.feature('os_bc_pvp', 'arena-mode'),
  onEnable() { /* … */ },
  onDisable() { /* … */ },
});
...
core.features.add('shop-integration', {
  condition: ctx => ctx.state.get('os_bc_shop', 'shopOpen') === true,
  onEnable() { /* … */ },
  onDisable() { /* … */ },
});
```
...
const shopFeatures = core.features.of<ShopFeatures>('os_bc_shop');
...
const balance = await core.rpc.request('os_bc_economy', 'getBalance', { player: 'Steve' });
...
core.register({ ..., translations: translationKeys });  // publish this addon's full by-locale map
core.translations.provide(translationKeys);   // or publish/replace later
