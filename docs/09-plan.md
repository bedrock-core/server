...
| 7 | **Delete** | `ScopedState`, `RemoteConfigAccessor` (now a thin wrapper over `core:db.*`), the interest-protocol design; docs site: `scoped-state.md` gone, `config.md` cross-addon section → query, `sync/state.md` labelled transport-level, new pages for observable / shared / db / query | 6 | 1 day |
