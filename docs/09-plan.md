...
| # | Question | Blocks |
| --- | --- | --- |
| ~~S5~~ | **ABI half answered 2026-09-05** — [spikes/S5](./spikes/S5-abi-survey.md): one direct ABI on world / entity / player / slot (six methods, batch, `Vector3`, 32 767 cap, ~10 µs set); `Dimension` has nothing; stackable `ItemStack` **throws** on write, non-stackable writes land on a copy; `ContainerSlot` writes cost 327 µs; `block_actor_dynamic_properties` sits on every block item. **Still open**: the mined block-entity item (`abi item`, needs 1.26.50) and the native DDUI observable items — equal-value `setData`, `subscribe` return, cost. | phase 1's `toNative` guard and disposal |
