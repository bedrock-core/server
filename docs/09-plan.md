...
| 4 | **`@bedrock-core/db`** — collections — *core done* | ✅ `db.collection` with `schema` (`version`/`defaults`/`migrate` lazy per document, quarantine under `#bad`), `accept`/`require` with the compile-time `Conflicts` check (document type named once in `schema<T>()` — TS infers all type arguments or none), validity gate re-resolving per op with proxied documents readable while unloaded, budget check + chunking on the direct ABI, identity cache, `subscribe`; 28 specs + a compiled type-assertion file. **Remaining**: `coalesce` opt-in with the parking rule; chunked index and `all(budget)`; `core:store_block` component for `onBreak`; `shared: true \| { as }` publishing; RPC `core:db.get/patch/set` with actor auth; `core.db` on the runtime (server-runtime files) | 3 | 2 days left |
