...
| 1 | **`@bedrock-core/observable`** | `observable` / `computed` / `effect` / `batch`, `ReadonlyObservable`, sync scheduling, listener isolation, vitest suite; ui-runtime `useObservable(obs, selector)`; `toNative` DDUI bridge as the `/minecraft` entry; config's `ChangeEmitter` replaced ([01-observable](./01-observable.md)) | S5's native-observable items | 2½ days |
| 2 | **`core.shared`** | rename with `core.state` as a deprecated alias one minor; `subscribe` on own and peer namespaces; `key()` returning an observable; owner-only apply filter with `open` keys; `persist` through the world host; framework announcements moved under the name; docs-site `scoped-state.md` → `shared.md` ([02-shared](./02-shared.md)) | 1 | 1½ days |
| 3 | **`@bedrock-core/db`** — resolver | `resolveHost(target)`: refusals, direct ABI, component ABI, proxied fallback; `DpHost` adapter; capability record; per-type cache that never caches a throw; `persistence.ts` reduced onto it, GameTests green ([03-db §Where a document can live](./03-db.md#where-a-document-can-live--capability-not-declaration)) | S5 | 1½ days |
...
| # | Question | Blocks |
| --- | --- | --- |
| S5 | ABI survey — for world, `Player`, `Entity`, `Dimension`, `ItemStack` (constructed and from a live slot), `ContainerSlot`, a block with `minecraft:block_entity`, a vanilla block, a mined block-entity item: which methods exist, every `/ynamic/` member on the prototype chain, both component ids, whether a write survives a fresh handle, enumeration, `Vector3`, `setDynamicProperties`, real byte budget by ladder, per-call cost. **Plus native DDUI observables**: does `setData` with an equal value notify and push; does `subscribe` return an unsubscribe handle; per-observable cost and any per-tick update cap. **Probe deployed for the ABI half; awaiting the run.** | phase 3's refusal list and capability records; phase 1's `toNative` guard and disposal |
